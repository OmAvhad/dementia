...
import "./personlizedSearchInput.css";
import { useState } from "react";
import axios from "axios";
import API from "../../api";

function PersonlizedSearchInput({pushChat}) {
  const [query, setQuery] = useState("");

  const getResponse = async (query) => {
    try {
        // axios post
        const response = await axios.get(API.local_test+"/personalised-query?query="+query, { message: query });
        console.log(response.data);
        // check if response.data is not empty by checking response.data.answer
        response.data.response = response.data.response.join("\n")
  ...
  const handlePushChat = async(query) => {
      pushChat(query, "sender");

        const data = await getResponse(query)
        console.log(data)
        if(data){
            pushChat(JSON.stringify({para: data.response, links: data.links}), "receiver")
        }
        
        setQuery("");
    }

    const initial_query = localStorage.getItem("initial_query");
    if(initial_query){
        localStorage.clear();
        handlePushChat(initial_query);
    }
...
export default PersonlizedSearchInput;
